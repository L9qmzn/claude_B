"""Smoke test for reading session history through the FastAPI endpoints.

Steps performed:

1. Ensure the target session metadata exists in `sessions.db` (so the API
   knows about it) – we insert/update if necessary.
2. Call `/sessions/{session_id}` to fetch the detailed view.
3. Call `/sessions` to confirm the summary listing includes the same session.

Usage example (assuming the FastAPI app is running on localhost:8207):

    python test_read_session_api.py \
        --base-url http://127.0.0.1:8207 \
        --cwd "C:/Users/11988/PycharmProjects/claude_B" \
        --session-id "83f757d2-2eaa-43c8-98e3-e269456be4ff" \
        --title "示例会话"
"""
...
def ensure_session_metadata(session_id: str, title: str, cwd: str) -> None:
...
def fetch_session_detail(base_url: str, session_id: str, auth: httpx.Auth) -> dict:
...
def fetch_session_list(base_url: str, auth: httpx.Auth) -> list[dict]:
...
def fetch_default_session_summary(base_url: str, auth: httpx.Auth) -> dict | None:
    sessions = fetch_session_list(base_url, auth)
...
def main() -> None:
    ...
    parser.add_argument(
        "--cwd",
        default=None,
        help="默认自动从 /sessions 读取 cwd",
    )
    parser.add_argument(
        "--session-id",
        default=None,
        help="默认自动从 /sessions 读取会话 ID",
    )
    ...
    parser.add_argument("--title", default="示例会话")
    ...
    if default_session:
        if session_id is None:
            ...
            print(f"ℹ️ 默认使用会话 {session_id}")
        if cwd is None:
            cwd = default_session["cwd"]
    elif session_id is None:
        print("⚠️ 没有可用 session，无法运行测试（请使用 --session-id 参数）。")
        return
    ...
    ensure_session_metadata(session_id, args.title, cwd)
    print(f"✔️ 确认 sessions.db 中存在元信息: {session_id}")

    detail = fetch_session_detail(args.base_url, session_id, auth)
    print("/sessions/{id} 返回:")
